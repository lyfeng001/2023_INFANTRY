...
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
...
#if USE_RMSTATE
    if (switch_is_mid(gimbal_control_set->gimbal_rc_ctrl->rc.s[0]))
    ...
    else if (switch_is_up(gimbal_control_set->gimbal_rc_ctrl->rc.s[0]))
    { // remote control 遥控器档
    ...
    if ((gimbal_control_set->gimbal_rc_ctrl->key.v & SWING_LEFT_KEY) == SWING_LEFT_KEY)
    {
        *yaw = yaw_channel * YAW_RC_SEN + 0.005f;
    }
    else if ((gimbal_control_set->gimbal_rc_ctrl->key.v & SWING_RIGHT_KEY) == SWING_RIGHT_KEY)
    {
        *yaw = yaw_channel * YAW_RC_SEN - 0.005f;
...
static void gimbal_relative_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
...
#if USE_RMSTATE
    if (switch_is_mid(gimbal_control_set->gimbal_rc_ctrl->rc.s[0])) // 键盘控制
    {
        if ((gimbal_control_set->gimbal_rc_ctrl->key.v & SWING_LEFT_KEY) == SWING_LEFT_KEY)
        {
            *yaw = 0.005f;
    ...
    else if (switch_is_up(gimbal_control_set->gimbal_rc_ctrl->rc.s[0])) // 遥控器控制
    {
        rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[YAW_CHANNEL], yaw_channel, RC_DEADBAND);
        rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], pitch_channel, RC_DEADBAND);
